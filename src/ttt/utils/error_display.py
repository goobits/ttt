...
def format_config_error(issue: str, file_path: Optional[str] = None) -> str:
    ...
    if file_path:
        return (
            f"⚙️  Config error in {file_path}\n"
            f"Issue: {issue}\n"
            f"Fix: Edit config or run 'ttt config list'"
        )
    
    return (
        f"⚙️  Configuration error\n"
        f"Issue: {issue}\n"
        f"Help: ttt config list"
    )
...
def format_rate_limit_error(provider: str, retry_after: Optional[int] = None) -> str:
    ...
    return (
        f"⚠️  Rate limit exceeded ({provider})\n"
        f"{wait_text}\n"
        f"Or try: ttt \"your prompt\" -m @fast  # Different model"
    )
...
def get_model_suggestions(invalid_model: str, available_models: Optional[List[str]] = None) -> List[str]:
    ...
    for model in available_models:
        ...
        if (invalid_lower in model_lower or 
            model_lower in invalid_lower or
            any(word in model_lower for word in invalid_lower.split("-"))):
...
def should_use_error_template(exception: Exception) -> bool:
    ...
    from ..core.exceptions import (
        APIKeyError, BackendConnectionError, BackendTimeoutError,
        ModelNotFoundError, RateLimitError, QuotaExceededError,
