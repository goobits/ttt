...
from ai.tools.recovery import (
    ErrorRecoverySystem, RetryConfig, InputSanitizer, 
    ErrorType, ErrorPattern, FallbackSuggestion
)
...
class TestErrorRecoverySystem:
    ...
    def test_should_retry_logic(self):
        ...
        assert recovery.should_retry(retryable_pattern, 3) == False  # Max attempts reached
    ...
    def test_calculate_retry_delay(self):
        ...
        recovery = ErrorRecoverySystem(RetryConfig(
            base_delay=1.0,
...
class TestIntegration:
    ...
    async def test_file_operation_with_recovery(self):
        ...
        assert ("File not found" in error_text or 
                "not found" in error_text or 
                "does not exist" in error_text or
                "Resource Error" in error_text)
    ...
    async def test_network_operation_with_retry(self):
        ...
        with patch('ai.tools.executor.get_tool', return_value=mock_tool):
            ...
            with patch.object(executor.recovery_system, '_adapt_arguments', return_value={"query": "test"}):
